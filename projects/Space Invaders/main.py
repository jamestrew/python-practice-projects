...
class Ship:
    ...
    def cooldown(self):
        if self.cool_down_counter >= self.COOLDOWN:
            self.cool_down_counter = 0
        elif self.cool_down_counter > 0:  # seems like redundant if check
...
def main():
    ...
    while run:
        ...
        if keys[pygame.K_w] and player.y - player_vel > 0:  # Up
            player.y -= player_vel
        if keys[pygame.K_s] and player.y + player_vel < HEIGHT - player.height - 20:  # Down
            player.y += player_vel
        ...
        for enemy in enemies[:]:
            ...
            shoot_prob = random.randrange(2, 4)
            if random.randrange(0, shoot_prob * FPS) == 1:
                enemy.shoot()
